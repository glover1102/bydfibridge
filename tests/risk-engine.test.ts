...
describe('RiskEngine', () => {
  ...
  it('rejects symbols without a loaded spec', () => {
    const config = createTestConfig();
    const engine = new RiskEngine(config, { getDailyPnl: () => 0 });

    expect(() => engine.getSymbolSpec('ETH-USDT')).toThrow(/No symbol spec loaded for ETH-USDT/);
  });

  it('rejects entries when no specs are loaded at runtime', () => {
    const config = createTestConfig();
    config.symbolSpecs = {};
    const engine = new RiskEngine(config, { getDailyPnl: () => 0 });
