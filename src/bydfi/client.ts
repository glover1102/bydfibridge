...
export class BydfiClient implements BydfiClientLike {
  constructor(private readonly config: Pick<AppConfig, 'bydfiApiKey' | 'bydfiApiSecret' | 'bydfiBaseUrl' | 'bydfiWallet' | 'bydfiSignatureHeader'>) {}
  ...
  private async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    const sanitizedResponseBody = this.sanitizeResponseBody(responseText);

    if (!response.ok) {
      throw new BydfiApiError(
        `BYDFi request failed for ${method} ${path} with status ${response.status}: ${sanitizedResponseBody}`,
    ...
    try {
      ...
      throw new BydfiApiError(
        `BYDFi returned a non-JSON success response for ${method} ${path}: ${sanitizedResponseBody}`,
        { path, method, status: response.status, responseBody: responseText }
      );
    }
    if (!payload || typeof payload !== 'object') {
    ...
    if ((payload.code !== undefined
      ...
      throw new BydfiApiError(
        `${payload.message ?? payload.msg ?? 'Unknown BYDFi API error'}: ${sanitizedResponseBody}`,
        { path, method, status: response.status, responseBody: responseText }
      );
    }
